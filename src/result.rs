...
pub fn show_result( site_list : &Vec<String>, cons_karlin96_list : &Vec<f64>, arg_c : &String )
{
	if ( *site_list ).len() != ( *cons_karlin96_list ).len() { error::error_bomb( "site_ent_len_not_same" ); }
	...
	if ( *arg_c ).as_str() == "yes" {
		...
		*/
		...
		println!( "num\tcons\tsite" );
		for i in 0 .. ( *cons_karlin96_list ).len() {
			println!( "{}\t{:.3}\t{}", i + 1, ( *cons_karlin96_list )[ i ], ( *site_list )[ i ] );
		}
	}
}

pub fn save_result( site_list : &Vec<String>, cons_karlin96_list : &Vec<f64>, arg_o : &String )
{
	...
	for i in 0 .. ( *cons_karlin96_list ).len() {
		writeln!( fout, "{}\t{:.3}\t{}", i + 1, ( *cons_karlin96_list )[ i ], ( *site_list )[ i ] ).expect( "FAILED to write" );
	}
